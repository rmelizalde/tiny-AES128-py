...
def AES128_CMAC_generate_subkey(K1, K2, key):
  Rb15 = 0x87
...
def AES128_CMAC(mac, message, msgLen, key):
    K1 = [0] * KEYLEN
    K2 = [0] * KEYLEN
    mLast = [0] * KEYLEN

    AES128_CMAC_generate_subkey(K1, K2, key)
    nrBlocks = msgLen/KEYLEN + (msgLen % KEYLEN != 0) #ceil(msgLen / KEYLEN)
    remainders = msgLen % KEYLEN
    ...
    if (padFlag): #pad the last block
        ...
        for i in range(0, KEYLEN):
            mLast[i] = message[msgLen-KEYLEN+i]
        for i in range(0, KEYLEN):
            mLast[i] ^= K1[i]
